...
// Ejercicio:
// Terminar la función para mostrar la imagen anterior
function prevSlide() {
        // Obtenemos todos los items de nuestro slider y los metemos en la variable itemsSlider
    // Es importante saber que document.getElementsByClassName() siempre nos devolverá un arreglo
    var itemsSlider = document.getElementsByClassName('slider-item')
    ...
    for(var index = 0; index < lengthItemsSlider; index++) {
        ...
        if(currentItem.classList.contains('active')) {
            ...
            // El anterior item será el que podemos obtener con el siguiente índice, por ejememplo:
            // si el item activo es el que se encuentra en el indice que no sea el 0 de nuestro array, el anterior item será el que
            // se encuentre en el índice 2 (ultimo-item)
            // Tenemos un caso donde lo anterior podría fallar:
            // si la longitud de mi slider es de 3 elementos, los índices serían 0, 1 y 2, en el caso de que el item activo fuera el
            // del índice 0, no podemos decir que el anterior es el del índice -1 porque no tenemos un elemento en el índice -1 y esto nos
            // daría un error, lo mejor es que al llegar al último item nos saltemos al ultimo que es el que está en el índice 2
            // resolvemos el problema mencionado anteriormente con la siguiente condición

            // si el índice es > 0, el anterior item será el que esté en el anterior índice
            if(index > 0) {
                prevItem = itemsSlider[index - 1]
            }
            // En el caso de que no se cumpla la condición anterior el item anterior será el que se encuentra en
            // el índice 2(el ultimo elemento)
            else {
                prevItem = itemsSlider[2]
            }
        }
    }
    // al item activo actualmente le quito la clase active
    activeItem.classList.remove('active')
    // al anterior item le agrego la clase active
